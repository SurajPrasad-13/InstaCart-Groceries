      ...
      <div className=" max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className=" grid md:grid-cols-2 gap-0">
            {/* left side Image */}
            <div className=" relative flex-center p-8 md:p-12 min-h-80 md:min-h-100 lg:min-h-120">
              <img
                src={product?.image}
                alt={product?.name}
            ...
            <div className=" p-6 md:p-10 flex flex-col justify-center">
                ...
                <div className="flex items-center gap-2 mb-5">
                  <div className="flex items-center gap-0.5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <StarIcon
                        key={star}
                        className={`size-4 ${star <= Math.round(product?.rating) ? "text-app-warning fill-app-warning" : "text-app-border"} `}
                      />
                    ))}
                  </div>
              ...
              <div className=" flex items-baseline gap-3 mb-5">
                <span className=" text-2xl sm:text-3xl md:text-4xl font-semibold text-app-green">
                  {currency}
                  {product?.price.toFixed(2)}{" "}
                </span>
                {product?.originalPrice > product?.price && (
                  <span className=" sm:text-lg text-app-text-light line-through">
                    {currency}
              ...
              <div className="mb-6">
                {product?.stock > 0 ? (
                  ...
                  <span className=" text-sm teapperr font-medium">
                    {" "}
                    Out of Stock
                  </span>
                )}
          ...
          <section className="mt-12 mb-44">
            ...
            <div className=" grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 xl:gap-8">
              {relatedProducts.slice(0, 5).map((rp) => (
                <ProductCard product={rp} key={rp._id} />
              ))}
            </div>
          </section>
        )}
